...
First, import the libraries that we'll need.
...
The first class is Webpage(). The objective is to open the Google Chrome browser invisibly using webdriver() and get the elements from the HTML of the specific website.
```python
class Webpage():
    def __init__(self):
        pass
...
The class, Window, is to create pop-out window.
```python
class Window():
...
This function aims to get the name of cities and areas in Taiwan.
```python
...
```
If the city was selected, this function will be triggered to show the areas.
```python
...
```
 If the user press the "藥局口罩剩餘查詢" button, it will scrape the pharmacies nearby and the remaining number of the masks.
```python
#if 口罩查詢 is pressed
def mask():
...
If the button "快篩院所查詢" is pressed, it will fetch the place providing the service of rapid test.
```python
#快篩院所
def testplace():
...
This function searches the hospitals offering people get vaccinated, when the users press the button, "疫苗接種院所查詢".
```python
#疫苗接種院所
def vaccineplace():
    vaccine = Webpage()
    ...
    t1.createpop(data)
```      
Grab the cases of COVID-19 infection when the users press "確診人數查詢"
```python
#covid-19 確診人數
...
def infection():
 ...
 ```
 The main function to show the window.
 ```python
